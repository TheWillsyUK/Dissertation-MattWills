...
class ThreatIntelligenceAnalyzerGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Cybersecurity Threat Intelligence Analysis Tool")

        self.analyzer = ThreatIntelligenceAnalyzer()

        self.industry_label = tk.Label(self.root, text="Please select your industry:")
        self.industry_label.pack()

        self.industry_var = tk.StringVar()
        self.industry_choices = list(self.analyzer.industries.keys())
        self.industry_menu = tk.OptionMenu(self.root, self.industry_var, *self.industry_choices)
        self.industry_menu.pack()
    ...
    def start_analysis(self):
        ...
        self.analyzer.ask_questions(industry)
        self.generate_report(industry)

    def generate_report(self, industry):
        report_text = f"Cybersecurity Assessment Report for the {industry} Industry:\n\n"
        improvements = []

        for aspect, questions in self.analyzer.industries[industry].items():
            aspect_score = sum(1 for answer in questions.values() if answer)
            if aspect_score == 0:
                improvements.append(f"{aspect}: High Priority (Red)")
            elif aspect_score <= len(questions) / 2:
        ...
        report_text += "Areas to prioritize based on the assessment:\n"
        for improvement in improvements:
            report_text += improvement + '\n'
        ...
        for aspect, questions in self.analyzer.industries[industry].items():
            for question, answer in questions.items():
                if answer is False:
                    report_text += f"\n{question}\n"
                    report_text += self.analyzer.get_suggestion(industry, aspect, question) + '\n'
                elif answer is True:
                    report_text += f"\n{question}\nNote: Good job! You have already implemented security procedures for this aspect.\n"
...
if __name__ == "__main__":
    app = ThreatIntelligenceAnalyzerGUI()
